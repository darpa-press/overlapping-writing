...
class GoogleDocument {
    ...
    getTableCellContent(content) {
        return content
            .map(({ paragraph }) =>
                paragraph.elements.map(this.formatText).join("")
            )
            .join("")
    ...
    processList(paragraph, index) {
        ...
        const prevList = _get(this.elements, [
    ...
    processFootnotes() {
        ...
        footnotes.sort(
            (footnote1, footnote2) =>
                parseInt(footnote1.value.number) -
                parseInt(footnote2.value.number)
        );
